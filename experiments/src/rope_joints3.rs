...
pub fn init_world(testbed: &mut Testbed) {
    ...
    let rigid_body =
        RigidBodyBuilder::dynamic().translation(vector![0.0, 0.3, 0.0]).gravity_scale(0.0);
    ...
    let rigid_body =
        ...
        .gravity_scale(0.0).linvel(vector![0.0,-2.0,0.0]);
    ...
    testbed.add_callback(move |_, physics_state, _, _| {
        ...
        if now.elapsed().unwrap().as_secs() >= 4 {
            let char = physics_state.bodies.get_mut(character_handle).unwrap();
            char.apply_impulse((child_t-char_t).normalize() * 0.0005, true);
            let child = physics_state.bodies.get_mut(child_handle).unwrap();
            child.apply_impulse((char_t-child_t).normalize() * 0.0005, true);
        }
    });
